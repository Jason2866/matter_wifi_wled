...
// ---- Shared status/light-state apply functions (used by SSE + polling) ----
...
// Poll helpers (fallback when SSE is unavailable)
function pollStatus() {
...
// POST /api/matter-reset
static esp_err_t handleMatterReset(httpd_req_t *req) {
  sendJson(req, "{\"ok\":true}");
  delay(500);
...
static StatusSnapshot takeStatusSnapshot() {
  StatusSnapshot s;
  s.connected          = isWiFiConnected();
  s.apMode             = apMode;
  s.matterStarted      = matterIsStarted();
  s.matterCommissioned = matterIsCommissioned();
...
static bool statusChanged(const StatusSnapshot& a, const StatusSnapshot& b) {
  return a.connected != b.connected
      || a.apMode != b.apMode
      || a.matterStarted != b.matterStarted
      || a.matterCommissioned != b.matterCommissioned
...
// Sends HTTP headers + initial events via raw socket, stores fd for async push.
// We use httpd_req_async_handler_begin() first, then raw socket writes for
// both headers and body.  This avoids chunked transfer encoding so that
// subsequent sseSendRaw() calls produce a clean continuous byte stream.
static esp_err_t handleEvents(httpd_req_t *req) {
  ...
  // Get the socket fd and create an async request copy BEFORE sending anything.
  // This prevents esp_http_server from closing the session when the handler returns.
  int fd = httpd_req_to_sockfd(req);
  httpd_req_t *asyncReq = nullptr;
  esp_err_t ret = httpd_req_async_handler_begin(req, &asyncReq);
  if (ret != ESP_OK) {
  ...
  // Build full HTTP response: headers + initial SSE events, sent as raw bytes.
  // No chunked transfer encoding — just a continuous stream.
  ...
  int sent = httpd_socket_send(httpServer, fd, resp.c_str(), resp.length(), 0);
  if (sent < 0 || (size_t)sent != resp.length()) {
...
static void ssePollAndPush() {
  ...
  // Keep the SSE session alive — update the LRU counter so esp_http_server
  // doesn't purge this long-lived connection when other clients connect.
  httpd_sess_update_lru_counter(httpServer, sseFd);
...
static void setupRoutes() {
  ...
  // POST /api/matter-reset
  static const httpd_uri_t matterResetUri = {
    .uri       = "/api/matter-reset",
    .method    = HTTP_POST,
    .handler   = handleMatterReset,
    .user_ctx  = nullptr
  };
  httpd_register_uri_handler(httpServer, &matterResetUri);
...
void webSetup() {
  ...
  // Increase recv timeout for large OTA uploads
  config.recv_wait_timeout = 30;
  // Generous send timeout for SSE keepalive on congested link
